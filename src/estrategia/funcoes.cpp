        ...
        if (pos[0].coluna <= pos[1].coluna)
            ...
            if (predicao_linha < campo->quinas[1].linha || predicao_linha >= campo->quinas[8].linha){
                ...
                if (predicao_linha < campo->quinas[1].linha){
                    ...
                    bate_coluna = ((float)campo->quinas[1].linha - b)/a;
                    aux = (-1)*(campo->quinas[3].coluna - bate_coluna)* tan(angulo) + campo->quinas[1].linha;
                    predicao_linha = aux;
                    ...
                    predicao_linha = (-1)*(campo->quinas[7].coluna - bate_coluna) * tan(angulo) + campo->quinas[8].linha;
        ...
        if (pos[0].coluna >= pos[1].coluna)
            ...
            if (predicao_linha < campo->quinas[0].linha || predicao_linha >= campo->quinas[9].linha){
                ...
                if (predicao_linha < campo->quinas[0].linha){
                    ...
                    aux = (bate_coluna - campo->quinas[15].coluna) * tan(angulo) + campo->quinas[0].linha;
                    predicao_linha = aux;
                    ...
                    predicao_linha = (bate_coluna - campo->quinas[14].coluna) * tan(angulo) +  campo->quinas[9].linha;
...
void estrategia_goleiro (Campo *campo, Posicao *pos_goleiro, Posicao *predicao_bola, Posicao pos_bola[], Jogador *jogadores_nossos, Jogador *jogadores_deles){
    ...
    float distancia_minima = (float)campo->num_colunas / 5.0;

    int adiantamento = 5;

    if (campo->nosso_lado == 1){

        centro_gol.coluna = (campo->quinas[3].coluna + campo->quinas[6].coluna)/2;
        centro_gol.linha = (campo->quinas[3].linha + campo->quinas[6].linha)/2;

    }
    else{

        centro_gol.coluna = (campo->quinas[14].coluna + campo->quinas[11].coluna)/2;
        centro_gol.linha = (campo->quinas[14].linha + campo->quinas[11].linha)/2;
    ...
    if (predicao_bola->linha == -1){ // caso tenha sido identificado a bolinha se movendo na direcao contratia do gol ou paralelamente

        if (distancia(centro_gol, pos_bola[0]) < distancia_minima)
            chuta(campo, jogadores_nossos, 0, jogadores_deles, pos_bola);
...
void estrategia1(Campo *campo, Jogador jogadores_nossos[], Jogador jogadores_deles[], Posicao pos_bola[], int quadrante_bola){
    ...
    estrategia_goleiro (campo, pos_goleiro, predicao_bola, pos_bola, jogadores_nossos, jogadores_deles);

    if (jogadores_nossos[0].trajetoria.num_passos == 1)
